...
export function getLedgerPage(params: LedgerQueryDTO) {
  return http.get<PageResult<LedgerDetailVO>>('/opr/ledgers', { params })
}

/** 台账详情 */
export function getLedgerById(id: number) {
  return http.get<LedgerDetailVO>(`/opr/ledgers/${id}`)
}

/** 双签确认 */
export function doubleSign(id: number) {
  return http.put(`/opr/ledgers/${id}/double-sign`)
}

/** 生成应收计划 */
export function generateReceivable(id: number) {
  return http.post(`/opr/ledgers/${id}/generate-receivable`)
}

/** 审核台账 */
export function auditLedger(id: number, data: { auditStatus: number; comment?: string }) {
  return http.put(`/opr/ledgers/${id}/audit`, data)
}

/** 手动推送应收 */
export function pushReceivable(id: number) {
  return http.post(`/opr/ledgers/${id}/push-receivable`)
}

/** 一次性首款录入 */
export function addOneTimePayment(id: number, data: unknown) {
  return http.post(`/opr/ledgers/${id}/one-time-payment`, data)
}

/** 查询应收计划列表 */
export function listReceivables(id: number) {
  return http.get<OprReceivablePlan[]>(`/opr/ledgers/${id}/receivables`)
}
...
export function searchLedgers(keyword?: string, pageSize = 10) {
  return http.get<LedgerSelectorVO[]>('/opr/ledgers/search', { params: { keyword, pageSize } })
}
